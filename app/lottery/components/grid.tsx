...
export default function LotteriesGrid({ items,pageKey,onCardClick }: Props) {
  ...
  return (
    <div key={pageKey} className='relative'>
      <div className="grid grid-cols-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 w-full max-w-screen-xl transition-transform duration-500 ease-in-out animate-slide">
        {items.map(item => (
          <LotteryPanel
            key={item.id}
            data={item}
